...
// Pembayaran
var pembayaran = [
    { "id": "via BRI", "norek": "987979798798 A/n Azid Zainuri (BRI)" },
    { "id": "via BNI", "norek": "987979798798 A/n Azid Zainuri (BNI)" },
    { "id": "via BCA", "norek": "987979798798 A/n Azid Zainuri (BCA)" },
    { "id": "via DANA", "norek": "987979798798 A/n Azid Zainuri (DANA)" }
];
...
function eventKabupaten(itemkab) {
    if (searchKabupaten === null) {
        $.ajax({
            url: "/experiment/lelang/kabupaten.json",
            dataType: "json",
            type: "GET",
    ...
    db.collection("dataproduk").orderBy("id", "desc").get().then((data) => {
        ...
        if (checkempty == false) {
            ...
            data.forEach(function (doc) {
                ...
                postHTML += "<div class='tombol-beli'><a href='/experiment/lelang/produk.html?id=" + id + "' class='beli-produk tombol'>Lihat</a></div>";
                postHTML += "</div>";
                postHTML += "</article>";
            });
